...
import { CAMPOS_META } from '../utils/pdfGenerator';
import { getStoredCoords, saveStoredCoords } from '../utils/coordsStorage';
...
export default function CalibracionPage() {
  ...
  const groups = CAMPOS_META.reduce((acc, m) => {
  ...
  useEffect(() => {
    const stored = getStoredCoords('cotizacion');
    if (stored) {
      setCoords(stored);
      return;
    }
    ...
    fetch('/api/coords')
      ...
      .then((data) => {
        setCoords(data);
        saveStoredCoords('cotizacion', data);
      })
      .catch(() => {
        import('../utils/pdfCoords.json').then((m) => {
          setCoords(m.default);
          saveStoredCoords('cotizacion', m.default);
        });
      });
  }, []);
  ...
  async function guardar() {
    ...
    try {
      ...
      saveStoredCoords('cotizacion', coords);
      setSaveState('ok');
      setTimeout(() => setSaveState('idle'), 3000);
    } catch (e) {
      const savedLocally = saveStoredCoords('cotizacion', coords);
      console.warn('No se pudo guardar por API, usando localStorage:', e);
      setSaveState(savedLocally ? 'ok' : 'error');
      setTimeout(() => setSaveState('idle'), savedLocally ? 3000 : 4000);
    }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col" style={{ height: '100vh' }}>
      ...
      <div className="bg-slate-800 border-b border-slate-700 px-4 py-2.5 flex items-center justify-between gap-3 flex-wrap flex-shrink-0">
        <div>
          <h1 className="font-bold text-yellow-400">Calibracion de coordenadas</h1>
          <p className="text-slate-400 text-xs">
            Haz clic en Posicionar y luego en el PDF donde debe ir cada dato.
          </p>
        </div>
        ...
        <div className="flex-1 overflow-auto bg-slate-950">
          <div className="sticky top-0 z-20 bg-slate-800/90 backdrop-blur border-b border-slate-700 px-4 py-1.5 flex items-center gap-4 text-xs font-mono">
            ...
            {picking && (
              <span className="ml-2 text-yellow-300 animate-pulse font-sans">
                Haz clic donde va: {CAMPOS_META.find((m) => m.key === picking)?.label}
              </span>
            )}
          </div>
          ...
          <div className="flex justify-center p-6 min-w-max">
            ...
            >
              {!pdfLoaded && !pdfError && (
                ...
                >
                  <div className="text-center">
                    ...
                    <p className="text-slate-400 text-sm">Cargando PDF...</p>
                  </div>
                </div>
              )}
              ...
              {pdfLoaded && CAMPOS_META.map((meta) => {
                const c = coords[meta.key];
              ...
              {lastSaved && (() => {
                ...
                const fieldMeta = CAMPOS_META.find((m) => m.key === lastSaved.key);
                return (
                  <div
                    style={{
                      position: 'absolute',
